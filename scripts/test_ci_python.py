...
def run_command(cmd, description, check=True):
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True, 
            text=True,
            env={**os.environ, 'PYTHONPATH': f"{os.environ.get('PYTHONPATH', '')}:{os.getcwd()}/python"}
        )
...
def simulate_ci_package_setup():
    ...
    # Test package import
    test_commands = [
        ("python -c \"import sys; sys.path.insert(0, 'python'); import demopy; print('✅ demopy module imported successfully')\"", "Test module import"),
        ("python -c \"import sys; sys.path.insert(0, 'python'); import demopy; print('Package version:', demopy.__version__)\"", "Check package version"),
    ]
...
def simulate_ci_testing():
    ...
    python_path = f"{os.getcwd()}/python"
    env = {**os.environ, 'PYTHONPATH': f"{os.environ.get('PYTHONPATH', '')}:{python_path}"}
    
    test_commands = [
        ("python -c \"import sys; sys.path.insert(0, 'python'); import demopy; print('✅ demopy module imported successfully')\"", "Pre-test import validation"),
...
"""
    ...
    success, _ = run_command(f"python -c \"{test_script}\"", "Test all demopy functionality", check=False)
