...
def install_pre_commit():
    ...
    success, _ = run_command("pre-commit --version", "Check pre-commit installation", check=False)
    if success:
        print("✅ pre-commit is already installed")
        return True
    
    # Install pre-commit
    success, _ = run_command("pip install pre-commit", "Install pre-commit package", check=False)
    if not success:
        print("❌ Failed to install pre-commit")
        return False
    
    # Verify installation
    success, _ = run_command("pre-commit --version", "Verify pre-commit installation", check=False)
    return success
...
def install_hooks():
    ...
    # Install hooks
    success, _ = run_command("pre-commit install", "Install pre-commit hooks", check=False)
    if not success:
        return False
    
    # Install commit message hooks (optional)
    success, _ = run_command("pre-commit install --hook-type commit-msg", "Install commit message hooks", check=False)
...
def update_hooks():
    ...
    success, _ = run_command("pre-commit autoupdate", "Update hook versions", check=False)
    return success
...
def test_hooks():
    ...
    success, output = run_command("pre-commit run --all-files", "Test all hooks", check=False)
    
    if success:
        print("✅ All hooks passed!")
    else:
        print("⚠️  Some hooks failed or made changes.")
        print("This is normal for the first run - hooks may auto-fix formatting issues.")
        print("\nHook output:")
        print(output)
        
        # Run again to see if issues are resolved
        print("\nRunning hooks again to verify fixes...")
        success2, _ = run_command("pre-commit run --all-files", "Re-test all hooks", check=False)
        if success2:
