...
def setup_precommit_hooks():
    ...
    success &= run_command("pre-commit install --hook-type commit-msg", "Install commit-msg hooks")
    
    # Run pre-commit on all files to set up the environment
    print("🔄 Running pre-commit on all files (this may take a while)...")
    run_command("pre-commit run --all-files", "Run pre-commit on all files", check=False)
...
"""
    ...
    try:
        ...
        run_command("git config commit.template .gitmessage", "Set commit message template")
