...
def test_maturin_development():
    ...
    success, _ = run_command("maturin --version", "Check maturin availability", check=False)
    if not success:
        print("⚠️  maturin not available, skipping development installation test")
        return True
    
    # Try maturin develop
    success, output = run_command("maturin develop", "Test maturin develop", check=False)
    ...
    if success:
        ...
        print("⚠️  maturin develop failed, this is expected in environments without Rust")
...
def test_package_installation_methods():
    ...
    methods = [
        ("PYTHONPATH approach", "export PYTHONPATH=$PYTHONPATH:$(pwd)/python && python -c 'import demopy; print(demopy.hello())'"),
        ("Direct path import", "python -c 'import sys; sys.path.insert(0, \"python\"); import demopy; print(demopy.hello())'"),
    ]
...
def validate_ci_setup():
    ...
    ci_commands = [
        ...
        ("python -c 'import sys; sys.path.insert(0, \"python\"); import demopy'", "Module import test"),
    ]
...
def main():
    ...
    print("3. Test import: python -c 'import sys; sys.path.insert(0, \"python\"); import demopy'")
